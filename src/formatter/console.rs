...
pub fn print_console(
    ...
    if reveal {
        eprintln!(
            "{}",
            "⚠️  WARNING: --reveal is set. Full values are visible in your terminal."
                .red()
                .bold()
        );
    }
    ...
    if visible.is_empty() {
        println!(
            "{}",
            "✅ Files match perfectly. No differences found."
                .green()
                .bold()
        );
        return;
    ...
    println!(
        "{}",
        "─".repeat(12 + 2 + key_width + 2 + lw + 2 + rw).dimmed()
    );
...
fn print_summary(diff: &DiffResult, show_all: bool) {
    ...
    if show_all && !diff.match_entries.is_empty() {
        parts.push(
            format!("{} match", diff.match_entries.len())
                .green()
                .to_string(),
        );
    } else if !show_all && !diff.match_entries.is_empty() {
        parts.push(
            format!("{} matching (hidden)", diff.match_entries.len())
                .dimmed()
                .to_string(),
        );
    }
    ...
    if diff.ok {
        ...
        println!(
            "{}",
            format!(
                "\n⚠️  {} missing key(s) — your app may break without them.",
                diff.missing.len()
            )
            .red()
            .bold()
